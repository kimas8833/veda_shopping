...
void ClientManager::displayInfo() {
    ...
    for (const auto& v : clientList) {
         Client* c = v.second;
         cout << setw(5) << setfill('0') << right << c->id() << " | ";          // ID 출력 (5자리)
         cout << setw(12) << setfill(' ') << left << c->getName() << " | ";    // 이름 출력 (12자리)
         cout << setw(14) << left << c->getPhoneNumber() << " | ";             // 전화번호 출력 (14자리)
         cout << setw(12) << left << c->getAddress() << endl;                  // 주소 출력 (12자리)
    }
}
